...
/* 顶部返回按钮组：返回所属分类页 / 返回地图 */
.topbar{display:flex;gap:10px;flex-wrap:wrap;margin:22px 0 0 28px;}
.back{
...
<div class="topbar">
  ...
  <a class="back" onclick="go('map.html')">🗺️ 返回地图</a>
</div>
...
<!-- 左右两侧的「上一家 / 下一家」浏览箭头 -->
<script src="../detail-nav.js"></script>
</body>
</html>
"""
        ...
        if os.path.abspath(img_src) != os.path.abspath(dest):
            if os.path.exists(dest):
                ...
                if same:
                    ...
                    _center_square_crop(dest)
            else:
                shutil.copyfile(img_src, dest)
                print(f"  图片已复制到 assets/img/{img_file}")
                _center_square_crop(dest)
    elif img_file:
        # 图片已经在 assets/img/ 里：原地按正中间裁剪成最大正方形
        _center_square_crop(os.path.join(BASE, "assets", "img", img_file))
